...
from twoman_protocol import LANE_BULK, LANE_CTL, LANE_PRI
...
class DnsLaneSelectionTests(unittest.TestCase):
    def test_helper_uses_control_lane_for_small_dns_payloads(self):
        stream = ProxyStream(DummyHelper(), 1, "1.1.1.1", 53)
        self.assertEqual(stream._data_lane(40), LANE_CTL)
    ...
    def test_agent_uses_control_lane_for_small_dns_payloads(self):
        stream = RemoteStream(DummyAgent(), 1)
        stream.target_host = "8.8.8.8"
        stream.target_port = 53
        self.assertEqual(stream._data_lane(64), LANE_CTL)
